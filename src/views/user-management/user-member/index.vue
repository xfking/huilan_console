...
const getList = () => {
  ...
  store.pageMember(query).then((res: any) => {
...
const handState = (row: any) => {
  ...
  store.stateMember({ id, state }).then((res) => {
...
<template>
  ...
  <ContentBox title="查询结果">
    ...
    <el-table
      ...
      style="width: 100%"
    >
      <el-table-column prop="id" min-width="150" label="用户编号" />
      <el-table-column prop="realname" min-width="150" label="用户名称" />
      <el-table-column prop="mobile" min-width="150" label="电话" />
      <el-table-column prop="email" min-width="150" label="邮箱" />
      <el-table-column prop="born_day" min-width="150" label="生日" />
      <el-table-column prop="add_tm" min-width="150" label="注册时间" />
